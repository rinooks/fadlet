...
export function useStages(boardId: string) {
  ...
  async function addStage(current: Stage[], title: string, durationSec: number) {
    const next: Stage = {
      id: genId(),
  ...
  async function updateStage(current: Stage[], id: string, patch: Partial<Pick<Stage, 'title' | 'durationSec'>>) {
